...
using namespace std;
using namespace cv;
//adding global variable for game subscriber
int GAS_normal_sup = 80;
int GAS_normal_pro = -170;
...
{  
    ...
    //in case subscriber isn't working
    GAS_normal_sup = 30;
    GAS_normal_pro = -150;
    if(angle_thumb>GAS_normal_sup && angle_thumb<(180-GAS_normal_sup)) up_down_flag = 1;
    else if(angle_thumb>GAS_normal_pro && angle_thumb<(-180-GAS_normal_pro)) up_down_flag = -1;
    else up_down_flag = 0;
...
{
    //message == str(GAS_normal_sup + "," str(GAS_normal_pro)
    ROS_INFO("I heard: [%s]", msg->data.c_str());
    CArray<CString,CString> GAS;
    CString field;
    int index = 0;
    while (AfxExtractSubString(field,msg,index,_T(',')))
    {
        GAS.Add(field);
        ++index;
    }
    CString sup = GAS.GetAt(0);
    CString pro = GAS.GetAt(1);
    GAS_normal_sup = _ttoi(sup); 
    GAS_normal_pro = _ttoi(pro);
    cout << "GAS_normal_sup: ";
    cin >> GAS_normal_sup
    cout << "GAS_normal_pro: ";
    cin >> GAS_normal_pro
...
int main(int argc, char **argv)
{
    ...
    //adding subscriber to get number from game
    //ros::Subscriber sub = n.subscribe("GAS",10,chatterCallback);
    
    string GASSCORES("GAS subscriber ran");
    printtf("%s\n",GASSCORES);
